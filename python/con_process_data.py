...
class isoProcess(object):
    ...
    def readonevideoFaceFile(self, filepath):
    ...
    @isValid: 1: valid ot test, 2:trin
    """  
    def readFaceFileCon(self, filepath = None, isValid = 2):
        if not os.path.exists(filepath):
            print filepath + " does not exists!"
            sys.exit()
            return None             

        allidVideo, allfaceX, allfaceY  = [], [], []
        width = self.width
        height = self.height
        labelNameSets = GetSubfileName(filepath)
        for labelName in labelNameSets:
            lableFilePath = filepath + "/" + labelName
            videoNameSets = GetSubfileName(lableFilePath)
            for videoName in videoNameSets:
                facepath = lableFilePath + "/" + videoName
                if isValid == 2:
                    videoID = int(labelName) * 10000 + int(videoName[: 4])
                elif isValid == 1:
                    videoID = int(videoName[: 4])
                else:
                    print("the parameter of isValid is wrong set")
                    sys.exit()
                faceX, faceY = self.readonevideoFaceFile(facepath)
                if faceX == 0:
                    faceX = width / 2
                    faceY = height / 2
                allfaceX.append(faceX)
                allfaceY.append(faceY)
                allidVideo.append(videoID)
        allidVideo = np.array(allidVideo)
        allfaceX = np.array(allfaceX)
        allfaceY = np.array(allfaceY)
        return allidVideo, allfaceX, allfaceY          
    """
    ...
    @saveFile: the saved new path
    @isValid: 1:valid or test, 2: train
    """
    def blackNonHandFace(self, oriImgFile = None, faceDetecionPath = None, saveFile = None, isValid = 2):
        width = self.width
        height = self.height
        if not os.path.exists(saveFile):
            strcmd = "mkdir -p " + saveFile
