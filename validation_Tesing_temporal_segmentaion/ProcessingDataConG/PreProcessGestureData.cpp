...
bool PreProcessGestureData::GetFileFacePosition()
{
	fstream readfile;

	readfile.open(faceDataPath, ios::in);
	if(!readfile)//file does not exist
	{
		faceX = 0;
		faceY = 0;
...
bool PreProcessGestureData::OutheadDetectionVIPLSDK(IplImage* colorImage, CString filename, int countframe)
{
...
bool PreProcessGestureData::cvtRGBToFrontRGB(CString colorPath, CString fdepthPath, CString outputfRGBPath)
{
	...
	capture = cvCreateFileCapture(fdepthPath);
	...
	{
		IplImage* tempFrame = cvQueryFrame(capture);
		vfDepth.push_back(cvCloneImage(tempFrame));
	}
	cvReleaseCapture(&capture);

	for(int i = 1;i < numFrames;i++)
		onevideoRectify(vColor[i]);

	int eps = 6;//image > eps is front ,else is background
	CString saveName = colorPath.Right(9);
	saveName = outputfRGBPath + "\\" + saveName;
	...
	{
		IplImage* tempImg = cvCreateImage(cvSize(vColor[k]->width, vColor[k]->height), vColor[k]->depth, 1);
		cvCvtColor(vfDepth[k], tempImg, CV_RGB2GRAY);	
		for(int i = 0; i < vfDepth[k]->height; i++)
			for(int j = 0; j < vfDepth[k]->width; j++)
			{
				int tmp = ((uchar*)(tempImg->imageData+i*tempImg->widthStep))[j];
				if(tmp < eps)
				{
					((uchar*)(vColor[k]->imageData + i * vColor[k]->widthStep))[j * 3] = 0;
					((uchar*)(vColor[k]->imageData + i * vColor[k]->widthStep))[j * 3 + 1] = 0;
					((uchar*)(vColor[k]->imageData + i * vColor[k]->widthStep))[j * 3 + 2 ] = 0;
				}
			}
		cvWriteFrame(writer, vColor[k]);
...
bool PreProcessGestureData::getForegroundRect(CString colorPath, CString depthPath, string outputPath)
{
	MyColorPath = colorPath;
	CvCapture *capture = cvCreateFileCapture(colorPath);
	...
	forgroundSave(outputPath);

	return true;
}

void PreProcessGestureData::forgroundSave(string outputPath)
{
	ofstream forgroundRect;
	string filename = outputPath;
	forgroundRect.open(filename, ios::out);
	...
	for(int k = 0; k < numFrames; k++)
	{
		...
		for(int i = 0; i < vDepth[k]->height; i++)
			...
			int theBox[100];
			int nThreshold=370;
			getConnexeCenterBox(dst, nMaxRect, theCent, theBox, nThreshold);
			//记录该帧前景区域坐标
			CString numID, tlx, tly, brx, bry;
			numID.Format("%04d", k);
			if(nMaxRect>=1)
			{
				tlx.Format("%03d", theBox[0]);
				tly.Format("%03d", theBox[1]);
				brx.Format("%03d", theBox[2]);
				bry.Format("%03d", theBox[3]);
				forgroundRect<<numID<<" "<<tlx<<" "<<tly<<" "<<brx<<" "<<bry<<endl;
			}
...
IplImage* PreProcessGestureData::ImgDiff(IplImage* img1, IplImage* img2)
{
	IplImage* tempImg1=cvCreateImage(cvSize(img1->width,img1->height),img1->depth,1);
	IplImage* tempImg2=cvCreateImage(cvSize(img2->width,img2->height),img2->depth,1);
	IplImage* tarImg=cvCreateImage(cvSize(img1->width,img1->height),img1->depth,1);
	cvCvtColor(img1, tempImg1, CV_RGB2GRAY);
	cvCvtColor(img2, tempImg2, CV_RGB2GRAY);
	for(int i=0;i<tarImg->height;i++)
		for(int j=0;j<tarImg->width;j++)
		{
			if(((uchar*)(tempImg2->imageData+i*tempImg2->widthStep))[j] > ((uchar*)(tempImg1->imageData+i*tempImg1->widthStep))[j])
				((uchar*)(tarImg->imageData+i*tarImg->widthStep))[j]
				=(((uchar*)(tempImg2->imageData+i*tempImg2->widthStep))[j]-((uchar*)(tempImg1->imageData+i*tempImg1->widthStep))[j]);
			else
				((uchar*)(tarImg->imageData+i*tarImg->widthStep))[j]=0;
			if(((uchar*)(tarImg->imageData+i*tarImg->widthStep))[j] < 50 || (((uchar*)(tempImg1->imageData+i*tempImg1->widthStep))[j]<30) || (((uchar*)(tempImg1->imageData+i*tempImg1->widthStep))[j]<30))
				((uchar*)(tarImg->imageData+i*tarImg->widthStep))[j]=0;
			else
				((uchar*)(tarImg->imageData+i*tarImg->widthStep))[j]=255;
		}
...
void PreProcessGestureData::getConnexeCenterBox(IplImage* image, int& nMaxRect, int* theCent, int* theBox, int nThreshold)
{
...
void PreProcessGestureData::backgroundRemove()
{
	...
	//CString TestSaveName("E:\\FaceFromHist.avi");
	//CvVideoWriter *TestWriter = 0;
	//TestWriter = cvCreateAVIWriter(TestSaveName, CV_FOURCC('M','P','4','2'), 10, cvSize(vColor[0]->width,vColor[0]->height), 1);
	for(int k = 0; k < numFrames; k++)
	{
		double newminDepth = 0;
		if(IsDtectFace)//如果检测到人脸的话，则根据平均灰度变化调节阈值,
		...
		GetFacePosiFromHist(dst, xface, yface);
		cvCircle(oneVedioImage, cvPoint(xface, yface), 10, CV_RGB(0, 0, 255));
		cvShowImage("TestFace", oneVedioImage);
		cvWaitKey(0);
		cvWriteFrame(TestWriter, oneVedioImage);
		//--------------------------------------------end
		*/
		//cvShowImage("ShowImage1", oneVedioImage);
		//cvWaitKey(0);
		cvCvtColor(oneVedioImage, tempImg, CV_RGB2GRAY);
		for(int i = 0; i < oneVedioImage->height; i++)
			for(int j = 0; j < oneVedioImage->width; j++)
...
void PreProcessGestureData::onevideoRectify(IplImage* bgrFrameL)
{
        double R1[3][3],R2[3][3],P1[3][4],P2[3][4];
...
void PreProcessGestureData::getFacePositionHist(CString filename)
{
