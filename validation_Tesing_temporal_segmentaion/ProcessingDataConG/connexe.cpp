			      ...
			      int *theDim )
{
  ...
  return( CountConnectedComponentsWithAllParams( inputBuf, typeIn, outputBuf, typeOut, theDim,
						 (double)200.0, /* default threshold */
						 _connectivity_,
					   ...
					   int outputIsBinary )
{
  ...
  if ( components == (typeConnectedComponent *)NULL ) {
    if ( (typeOut != CONN_USHORT) && (typeOut != CONN_SSHORT) ) free( tmpBuf );
    if ( _VERBOSE_)
  ...
  if ( nbFoundCC <= 0 ) {
    ...
    case CONN_SCHAR :
      break;
    case CONN_USHORT :   v *= sizeof( u16 );   break;
    case CONN_SSHORT :   v *= sizeof( s16 );   break;
    case CONN_FLOAT  :   v *= sizeof( r32 );   break;
    default :
      if ( _VERBOSE_ ) {
...
int GetCenterofComponent( void *bufferOut , int *theDim , int *theCent , int nmax , BOOL bConvert)
{
	int dimx = theDim[0] , dimy = theDim[1] ;
	memset( theCent, 0 , 2*nmax * sizeof(int) ) ;
	int *theNum = new int[nmax];	
	memset( theNum, 0 , nmax * sizeof(int) ) ;
	int i=0, j=0;
	int b=0,bmax =0 ;
	for ( i =0 ; i<dimx ; i++ )
		for (j =0 ; j<dimy ; j++ )
		{
			b = *( (unsigned char *)bufferOut + i * dimy + j ) ;
			if (b>0)	{
				*( theCent + 2*b-2 ) = *( theCent + 2*b-2 ) + i;
				*( theCent + 2*b-1 ) = *( theCent + 2*b-1 ) + j;
				theNum[b-1] = theNum[b-1] + 1;				
			}
			if (b>0 && bConvert)	*( (unsigned char *)bufferOut + i * dimy + j ) = 255;
		}
	for ( i=0 ; i<nmax ; i++)
	{
		if (theNum[i]>0)	{
			*(theCent + 2*i) = *(theCent + 2*i) / theNum[i] ;
			*(theCent + 2*i+1) = *(theCent + 2*i+1) / theNum[i] ;
		}
	}	
...
int GetCenterofComponentWithBox( void *bufferOut , int *theDim , int *theCent , int *theBox , int nmax , BOOL bConvert ) 
{
	int dimx = theDim[0] , dimy = theDim[1] ;
	memset( theCent, 0 , 2*nmax * sizeof(int) ) ;
	int *theNum = new int[nmax];	
	...
	for ( i= 0 ; i<nmax ; i++)
	{
		theBox[i*4 ]    = 10000;	theBox[i*4 + 1] = 10000;
		theBox[i*4 +2 ] = 0;		theBox[i*4 + 3] = 0;

	}
	// 0 -mininum of row index 1- minimum of column index 2-max of row index ,3 - max of column index

	for ( i =0 ; i<dimx ; i++ )
		for (j =0 ; j<dimy ; j++ )
		{
//			b = *( (unsigned char *)bufferOut + i * dimy + j ) ;
			b = *( (unsigned char *)bufferOut + j * dimx + i ) ;

			if (b>0)	{
				*( theCent + 2*b-2 ) = *( theCent + 2*b-2 ) + i;
				*( theCent + 2*b-1 ) = *( theCent + 2*b-1 ) + j;
				theNum[b-1] = theNum[b-1] + 1;	
				if ( i< theBox[4*(b-1) ])		theBox[4*(b-1) ] = i ;
				if ( j< theBox[4*(b-1) +1 ])	theBox[4*(b-1) + 1] = j ;
				if ( i>theBox[4*(b-1) +2 ])		theBox[4*(b-1) +2 ] = i ;
				if ( j>theBox[4*(b-1) +3 ])		theBox[4*(b-1) +3 ] = j ;
			}
//			if (b>0 && bConvert)	*( (unsigned char *)bufferOut + i * dimy + j ) = 255;
			if (b>0 && bConvert)	*( (unsigned char *)bufferOut + j * dimx + i ) = 255;
...
int GetCenterofComponentWithBoxNew( void *bufferOut , int *theDim , int *theCent , int *theBox , int nmax , BOOL bConvert, int nthreshold )
{
	if(nthreshold < 0)
		nthreshold = 1; //the minimum of region points
...
//			if (b>0 && bConvert)	*( (unsigned char *)bufferOut + i * dimy + j ) = 255;
	...
	for(i  = 0; i < nmax; i ++)
	{
		int tempNum = nIndexNo[i];
		if(nIndexNo[i] > nthreshold)
			nRect ++;
