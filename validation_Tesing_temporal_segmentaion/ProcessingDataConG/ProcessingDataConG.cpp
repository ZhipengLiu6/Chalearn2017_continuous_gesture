...
//ValidPath: the path of RGB valid or test dataset
//HandLabelPath: the path of hand detection results
//IsoVideoSaveName: the path of segmented RGB video 
//IsoDepthVideoSavePath: the path of segmented aligned depth video 
//IsoHandDetectionSavePath: the path of segmented hand detection.
//oriDepthVideo: the path of aligned depth valid or test dataset.
//SegInfopath: the temporal segmentaion information
//videoLenFile: the path of recording video lenth
CString ValidPath = "..\\data\\ConGD_phase_1\\valid"; 
CString HandLabelPath = "..\\data\\cong\\valid";
CString IsoVideoSavePath = "..\\output\\seg_valid_rgb_2stream";
CString IsoDepthVideoSavePath = "..\\output\\seg_valid_aligned_depth_2stream";
CString IsoHandDetectionSavePath = "..\\output\\seg_valid_handDetection_2stream";
CString oriDepthVideo = "..\\data\\ConGD_Phase_1_aligned\\valid";
...
CString faceTxtFilename = "C:\\face.txt";
...
int main()
{
	...
	ndevel = getCurrentDir(ValidPath.GetBuffer(0)) ;
	...
	for(int i = 1; i <= ndevel; i++)
	{
		...
		CSDevel.Format("%03d", i);
		CString ValidDevelPath = ValidPath + "\\" + CSDevel;
		CString IsoVideoDevelPath = IsoVideoSavePath + "\\" + CSDevel;
	...
	_mkdir(IsoHandDetectionSavePath);

	//vector<string> videoPath;
	ndevel = getCurrentDir(HandLabelPath.GetBuffer(0)) ;
	for(int i = 1; i <= ndevel; i++)
	{
		CString CSDevel;
		CSDevel.Format("%03d", i);
		CString ValidDevelPath = HandLabelPath + "\\" + CSDevel;
		CString IsoVideoDevelPath = IsoHandDetectionSavePath + "\\" + CSDevel;
		CString SegInfoDevelpath = SegInfopath + "\\" + CSDevel;
	...
	_mkdir(IsoDepthVideoSavePath);

	//vector<string> videoPath;
	ndevel = getCurrentDir(oriDepthVideo.GetBuffer(0)) ;
	for(int i = 1; i <= ndevel; i++)
	{
		CString CSDevel;
		CSDevel.Format("%03d", i);
		CString ValidDevelPath = oriDepthVideo + "\\" + CSDevel;
		CString IsoVideoDevelPath = IsoDepthVideoSavePath + "\\" + CSDevel;
		CString SegInfoDevelpath = SegInfopath + "\\" + CSDevel;
