...
def parse_args():
    ...
    parser.add_argument('--dataset', dest='dataset', help='set the dataset of IsoGD then do hand detection',
                        default='ConGD_Phase_1/convertContinousToIsoGestrueTrain_new')
    ...
    for video_folder in video_folders:
        if float(video_folder)>=args.begin and float(video_folder)<=args.end:
            if(os.path.isdir(os.path.join(root_path, video_folder))==True):
                ...
                for video in videos:
                    if(video[-5:-4]=='M'):    #Process the XXXX.M.avi(XXXXX.M.avi) and its corresponding XXXX.K.avi(XXXXX.K.avi)
                        rgb_video = video
                        # result txt_file
                        index = len(video)-5
                        depth_video = video[0:index]+'K.avi'
                        print rgb_video
                        txt_path = os.path.join(output_path, video_folder, 'Label_'+video[0:index]+'M.txt')
